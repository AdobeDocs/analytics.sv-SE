...
Ett villkor för en användaragent kontrollerar användaragentvärdet för att se om det **[!UICONTROL starts with]** eller **[!UICONTROL contains]** den angivna strängen är det. Om **[!UICONTROL contains]** är markerat matchas delsträngen om den förekommer någonstans i användaragenten.

Valfria värden kan inkluderas i listan för att definiera värden som användaragenten inte får innehålla för att matchningen ska lyckas. **[!UICONTROL does not contain]** Du kan ange flera värden genom att inkludera ett värde per rad. Om användaragenten uppfyller villkoren som anges i matchningssträngen, men även innehåller en sträng i listan som inte innehåller någon, betraktas den inte som en matchning.

Fältet får inte innehålla fler än 100 tecken. **[!UICONTROL contains]** Listan innehåller inte mer än 255 tecken minus ett avgränsningstecken för varje ny rad. (Detta är lika med antalet strängar - 1. Om du anger 4 *som inte innehåller* strängar krävs tre avgränsningstecken.) Alla strängmatchningar är inte skiftlägeskänsliga.
...
1. Klicka **[!UICONTROL Add Rule]** och definiera ett eller flera matchningsvillkor.
...
|--- |--- |---|---|---|
| Punktnamn | IP-start | IP-slut | Matchningsregel<br>för agent (innehåller eller börjar med)</br> | Agenten exkluderar<br>(högst 255 tecken)</br> |
...
* **IP-intervallmatchning**: Definiera ett intervall med IP-adresser med kolumnerna IP Start och IP End. Jokertecken kan användas för att matcha IP-intervall, till exempel `10.10.10.*` för `10.10.20.*`. Matchningsregel, Inkludera agent och Exkludera agent måste vara tomma.
...
Markera **[!UICONTROL Overwrite existing rules]** kryssrutan om du vill ta bort alla befintliga regler och ersätta dem med reglerna som är definierade i överföringsfilen.

...
| `browser` | Numeriskt ID för webbläsaren. Refererar till `browser.tsv` uppslagstabellen. | int unsigned |
...
| `c_color` | Färgpalettens bitdjup. Används som en del av beräkningen av [färgdjupsdimensionen](/help/components/dimensions/color-depth.md) . AppMeasurement använder JavaScript-funktionen `screen.colorDepth()`. | char(20) |
| `campaign` | Variabel som används i [spårningskoddimensionen](/help/components/dimensions/tracking-code.md) . | varchar(255) |
| `carrier` | Adobe Advertising Cloud integrationsvariabel. Anger mobiloperatör. Refererar till `carrier` uppslagstabellen. | varchar(100) |
| `channel` | Variabel som används i dimensionen för [platsavsnitt](/help/components/dimensions/site-section.md) . | varchar(100) |
...
| `color` | ID för färgdjup baserat på `c_color` kolumnens värde. Refererar till `color_depth.tsv` uppslagstabellen. | smallint unsigned |
| `connection_type` | Numeriskt ID som representerar anslutningstypen. Variabel som används i dimensionen för [anslutningstypen](/help/components/dimensions/connection-type.md) . Refererar till `connection_type.tsv` uppslagstabellen. | tinyint unsigned |
| `cookies` | Variabel som används i [Cookie-supportdimensionen](/help/components/dimensions/cookie-support.md) .<br>Y:<br>EnabledN:<br>DisabledU: Okänd | char(1) |
| `country` | Numeriskt ID som representerar det land som träffen kom från. Används i dimensionen [Länder](/help/components/dimensions/countries.md) . Använder `country.tsv` sökning. | smallint unsigned |
| `ct_connect_type` | Relaterat till `connection_type` kolumnen. De vanligaste värdena är LAN/Wifi, mobiloperatör och modem. | char(20) |
...
| `domain` | Variabel som används i [domändimensionen](/help/components/dimensions/domain.md) . Baserat på besökarens internetanslutning. | varchar(100) |
...
| `geo_city` | Namnet på den stad som träffen kom från, baserat på IP. Används i [stadsdimensionen](/help/components/dimensions/cities.md) . | char(32) |
| `geo_country` | Förkortning av det land som träffen kom från, baserat på IP. | char(4) |
| `geo_dma` | Numeriskt ID för det demografiska område som träffen kom från, baserat på IP. Används i DMA-dimensionen för [USA](/help/components/dimensions/us-dma.md) . | int unsigned |
...
| `geo_zip` | Postnumret som träffen kom från baseras på IP. Hjälper till att fylla i dimensionen för [postnummer](/help/components/dimensions/zip-code.md) . Se även `zip`. | varchar(16) |
...
| `language` | Numeriskt ID för språk. Använder `languages.tsv` uppslagstabell. | smallint unsigned |
| `last_hit_time_gmt` | Tidsstämpel (i Unix-tid) för föregående träff. Används för att beräkna [Dagar sedan senaste besök](/help/components/dimensions/days-since-last-visit.md) . | int |
| `last_purchase_num` | Variabel som används i [kundlojalitetsdimensionen](/help/components/dimensions/customer-loyalty.md) . Antalet tidigare inköp som besökaren har gjort. <br>0: Inga tidigare inköp (inte en kund) <br>1: 1 inköp (ny kund) <br>2: 2 tidigare inköp (returkund) <br>3: 3 eller fler tidigare inköp (lojal kund) | int unsigned |
| `last_purchase_time_gmt` | Används i dimensionen [Dagar sedan senaste köp](/help/components/dimensions/days-since-last-purchase.md) . Tidsstämpel (i Unix-tid) för det senaste köpet. För förstagångsköp och besökare som inte har gjort något inköp tidigare är det här värdet `0`. | int |
...
| `mobileaction` | Mobilåtgärd. Samlas automatiskt in när `trackAction` anropas i mobiltjänster. Tillåter automatisk åtgärdspunkt i appen. | varchar(100) |
...
| `os` | Numeriskt ID som representerar besökarens operativsystem. Baserat på `user_agent` kolumnen. Använder `os` sökning. | int unsigned |
...
| `page_type` | Används för att fylla i dimensionen för [sidor som inte hittades](/help/components/dimensions/pages-not-found.md) . Används endast för 404 sidor. Variabeln ska antingen vara tom eller innehålla värdet `ErrorPage`. | char(20) |
| `page_url` | URL:en för träffen. Borttagen från begäran om länkspårningsbild. | varchar(255) |
| `pagename` | Används för att fylla i [siddimensionen](/help/components/dimensions/page.md) . Om [`pagename`](/help/implement/vars/page-vars/pagename.md) variabeln är tom används `page_url` i stället Analytics. | varchar(100) |
...
| `persistent_cookie` | Används i dimensionen [Stöd](/help/components/dimensions/persistent-cookie-support.md) för beständig cookie. Anger om besökaren stöder cookies som inte tas bort efter varje träff. | char(1) |
...
| `product_list` | Produktlistan har skickats via [`products`](/help/implement/vars/page-vars/products.md) variabeln. Produkterna avgränsas med kommatecken medan de enskilda produktegenskaperna avgränsas med semikolon. | text |
| `product_merchandising` | Används inte. Använd `product_list` istället. | text |
| `prop1` - `prop75` | Anpassade trafikvariabler 1-75. Används i [Prop](/help/components/dimensions/prop.md) -dimensioner. | varchar(100) |
| `purchaseid` | Unik identifierare för ett köp, enligt inställningen med [`purchaseID`](/help/implement/vars/page-vars/purchaseid.md) variabeln. Används av `duplicate_purchase` kolumnen. | char(20) |
| `quarterly_visitor` | Flagga som avgör om träffen är en ny kvartalsbesökare. | tinyint unsigned |
| `ref_domain` | Baserat på referenskolumnen. Den refererande domänen för träffen. Används i dimensionen [Refererande domän](/help/components/dimensions/referring-domain.md) . | varchar(100) |
| `ref_type` | Ett numeriskt ID som representerar typen av referens för träffen. Används i [referensdimension](/help/components/dimensions/referrer-type.md) . <br>1: Inuti din webbplats<br>2: Andra webbplatser <br>3: Sökmotorer <br>4: Hårddisk <br>5: USENET <br>6: Typed/Bookmarked (ingen referent) <br>7: E-post <br>8: Ingen JavaScript <br>9: Sociala nätverk | tinyint unsigned |
| `referrer` | Föregående sidas URL. Används i [referensdimensionen](/help/components/dimensions/referrer.md) . Observera, att medan `referrer` datatypen varchar(255) används `post_referrer` datatypen varchar(244). | varchar(255) |
| `resolution` | Numeriskt ID som representerar bildskärmens upplösning. Används i dimensionen [Bildskärmsupplösning](/help/components/dimensions/monitor-resolution.md) . Använder `resolution.tsv` uppslagstabell. | smallint unsigned |
...
| `sampled_hit` | Används inte längre. Användes tidigare för provtagning i Ad Hoc Analysis. | char(1) |
...
| `search_page_num` | Används av [dimensionen Rankning](/help/components/dimensions/all-search-page-rank.md) för alla söksidor. Anger vilken sida av sökresultat din webbplats hade innan användaren klickade igenom till din webbplats. | smallint unsigned |
...
| `service` | Används inte. Använd `page_event` istället. | char(2) |
...
| `tnt_post_vista` | Används inte längre. Använd `post_tnt` istället. | text |
| `transactionid` | En unik identifierare där olika datapunkter kan överföras senare via datakällor. Samlas in med hjälp av [`transactionID`](/help/implement/vars/page-vars/transactionid.md) variabeln. | text |
...
| `user_hash` | Inte till användning. Hash för rapportens Suite-ID. Använd `username` istället. | int unsigned |
| `user_server` | Används i [serverdimensionen](/help/components/dimensions/server.md) . | varchar(100) |
| `userid` | Inte till användning. Det numeriska ID:t för rapportsvitens ID. Använd `username` istället. | int unsigned |
| `username` | Rapportsvitens ID för träffen. | char(40) |
| `va_closer_detail` | Variabel som används i dimensionen [Senaste beröringsdetalj](/help/components/dimensions/last-touch-detail.md) . | varchar(255) |
| `va_closer_id` | Numeriskt ID som identifierar dimensionen för den [sista beröringskanalen](/help/components/dimensions/last-touch-channel.md) . En sökning efter detta ID finns i Marketing Channel Manager. | tinyint unsigned |
| `va_finder_detail` | Variabel som används i dimensionen [Första beröringsdetalj](/help/components/dimensions/first-touch-detail.md) . | varchar(255) |
| `va_finder_id` | Numeriskt ID som identifierar dimensionen för den [första beröringskanalen](/help/components/dimensions/first-touch-channel.md) . En sökning efter detta ID finns i Marketing Channel Manager. | tinyint unsigned |
| `va_instance_event` | Flagga för att identifiera [instanser](/help/components/metrics/instances.md)av marknadsföringskanal. | tinyint unsigned |
| `va_new_engagement` | Flagga för att identifiera Marketing Channel [New engagement](/help/components/metrics/new-engagements.md). | tinyint unsigned |
...
| `visid_type` | Ej för extern användning. som används internt av Adobe för att bearbeta optimeringar. Numeriskt ID som representerar den metod som används för att identifiera besökaren.<br>0: Anpassat besökar-ID eller Okänt/ej tillämpligt<br>1: IP-adress och reserv för användaragent <br>2: HTTP Mobile Subscriber Header <br>3: Äldre cookie-värde (`s_vi`) <br>4: Fallback-cookie-värde (`s_fid`) <br>5: Identitetstjänst | tinyint unsigned |
| `visit_keywords` | Variabel som används i [söknyckelordsdimensionen](/help/components/dimensions/search-keyword.md) . I den här kolumnen används en icke-standard teckengräns på varchar(244) för att rymma den serverlogik som används av Adobe. | varchar(244) |
| `visit_num` | Variabel som används i dimensionen för [besöksnumret](/help/components/dimensions/visit-number.md) . Börjar vid 1 och ökar stegvis varje gång ett nytt besök påbörjas per besökare. | int unsigned |
| `visit_page_num` | Variabel som används i [träffdjupsdimensionen](/help/components/dimensions/hit-depth.md) . Ökar med 1 för varje träff som användaren skapar. Återställer varje besök. | int unsigned |
| `visit_ref_domain` | Baserat på `visit_referrer` kolumnen. Den första refererande domänen för besöket. | varchar(100) |
| `visit_ref_type` | Numeriskt ID som representerar referenstypen för besökets första referent. Använder `referrer_type.tsv` uppslagstabellen. | tinyint unsigned |
...
| `zip` | Hjälper till att fylla i dimensionen för [postnummer](/help/components/dimensions/zip-code.md) . Se även `geo_zip`. | varchar(50) |

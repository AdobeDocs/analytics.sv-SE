...
| `browser` | Numeriskt ID för webbläsaren. Refererar till uppslagstabellen `browser.tsv`. | int unsigned |
...
| `campaign` | Variabel som används i [spårningskoden](/help/components/dimensions/tracking-code.md)-dimensionen. | varchar(255) |
| `carrier` | Adobe Advertising Cloud integrationsvariabel. Anger mobiloperatör. Refererar till uppslagstabellen `carrier`. | varchar(100) |
| `channel` | Variabel som används i dimensionen [Platsavsnitt](/help/components/dimensions/site-section.md). | varchar(100) |
...
| `color` | ID för färgdjup baserat på värdet för kolumnen `c_color`. Refererar till uppslagstabellen `color_depth.tsv`. | smallint unsigned |
...
| `cookies` | Variabel som används i [Cookie-stödet](/help/components/dimensions/cookie-support.md)-dimensionen.<br>Y: <br>EnabledN: <br>DisabledU: Okänd | char(1) |
...
| `domain` | Variabel som används i dimensionen [Domän](/help/components/dimensions/domain.md). Baserat på besökarens internetanslutning. | varchar(100) |
...
| `ef_id` | `ef_id` som används i Adobe Advertising Cloud-integreringar. | varchar(255) |
| `evar1 - evar250` | Egna variabler 1-250. Används i dimensionerna [eVar](/help/components/dimensions/evar.md). Varje organisation använder eVars på olika sätt. Det bästa stället att få mer information om hur er organisation fyller i respektive eVars är ett dokument som är specifikt för er organisation. | varchar(255) |
...
| `geo_dma` | Numeriskt ID för det demografiska område som träffen kom från, baserat på IP. Används i dimensionen [US DMA](/help/components/dimensions/us-dma.md). | int unsigned |
...
| `geo_zip` | Postnumret som träffen kom från baseras på IP. Hjälper till att fylla i dimensionen [Postnummer](/help/components/dimensions/zip-code.md). Se även `zip`. | varchar(16) |
...
| `last_hit_time_gmt` | Tidsstämpel (i Unix-tid) för föregående träff. Används för att beräkna [Dagar sedan senaste besök](/help/components/dimensions/days-since-last-visit.md)-dimensionen. | int |
| `last_purchase_num` | Variabel som används i dimensionen [Kundlojalitet](/help/components/dimensions/customer-loyalty.md). Antalet tidigare inköp som besökaren har gjort. <br>0: Inga tidigare inköp (inte en kund)  <br>1: 1 inköp (ny kund)  <br>2: 2 tidigare inköp (returkund)  <br>3: 3 eller fler tidigare inköp (lojal kund) | int unsigned |
...
| `mobilemessagepushoptin` | Samlades in från kontextdatavariabeln `a.push.optin`. Ange som&quot;true&quot; när användaren väljer att skicka meddelanden; i annat fall är värdet &quot;false&quot;. | varchar(255) |
...
| `mobileosenvironment` | Samlades in från kontextdatavariabeln `a.OSEnvironment`. Lägesmiljö, t.ex. Android eller iOS. | varchar(255) |
...
| `persistent_cookie` | Används i dimensionen [Stöd för beständig cookie](/help/components/dimensions/persistent-cookie-support.md). Anger om besökaren stöder cookies som inte tas bort efter varje träff. | char(1) |
...
| `product_list` | Produktlistan har skickats in via variabeln [`products`](/help/implement/vars/page-vars/products.md). Produkterna avgränsas med kommatecken medan de enskilda produktegenskaperna avgränsas med semikolon. | text |
| `product_merchandising` | Används inte. Använd `product_list` i stället. | text |
| `prop1` -  `prop75` | Anpassade trafikvariabler 1-75. Används i dimensionerna [Prop](/help/components/dimensions/prop.md). | varchar(100) |
...
| `ref_domain` | Baserat på referenskolumnen. Den refererande domänen för träffen. Används i [referensdomänen](/help/components/dimensions/referring-domain.md)-dimensionen. | varchar(100) |
| `ref_type` | Ett numeriskt ID som representerar typen av referens för träffen. Används i [referenstypen](/help/components/dimensions/referrer-type.md)-dimensionen. <br>1: Inuti din webbplats<br>2: Andra webbplatser  <br>3: Sökmotorer  <br>4: Hårddisk  <br>5: USENET  <br>6: Typed/Bookmarked (ingen referent)  <br>7: E-post  <br>8: Ingen JavaScript  <br>9: Sociala nätverk | tinyint unsigned |
| `referrer` | Föregående sidas URL. Används i [referensdimensionen](/help/components/dimensions/referrer.md). Observera att medan `referrer` använder datatypen varchar(255) använder `post_referrer` datatypen varchar(244). | varchar(255) |
...
| `search_page_num` | Används av dimensionen [Alla söksidor rankas](/help/components/dimensions/all-search-page-rank.md). Anger vilken sida av sökresultat din webbplats hade innan användaren klickade igenom till din webbplats. | smallint unsigned |
...
| `va_closer_id` | Numeriskt ID som identifierar dimensionen [Senaste beröringskanal](/help/components/dimensions/last-touch-channel.md). En sökning efter detta ID finns i Marketing Channel Manager. | tinyint unsigned |
| `va_finder_detail` | Variabel som används i dimensionen [First touch detail](/help/components/dimensions/first-touch-detail.md). | varchar(255) |
| `va_finder_id` | Numeriskt ID som identifierar dimensionen [Första beröringskanalen](/help/components/dimensions/first-touch-channel.md). En sökning efter detta ID finns i Marketing Channel Manager. | tinyint unsigned |
...
| `visit_keywords` | Variabel som används i [söknyckelordet](/help/components/dimensions/search-keyword.md)-dimensionen. I den här kolumnen används en icke-standard teckengräns på varchar(244) för att rymma den serverlogik som används av Adobe. | varchar(244) |
| `visit_num` | Variabel som används i dimensionen [Besöksnummer](/help/components/dimensions/visit-number.md). Börjar vid 1 och ökar stegvis varje gång ett nytt besök påbörjas per besökare. | int unsigned |
| `visit_page_num` | Variabel som används i dimensionen [Träffdjup](/help/components/dimensions/hit-depth.md). Ökar med 1 för varje träff som användaren skapar. Återställer varje besök. | int unsigned |
| `visit_ref_domain` | Baserat på kolumnen `visit_referrer`. Den första refererande domänen för besöket. | varchar(100) |
| `visit_ref_type` | Numeriskt ID som representerar referenstypen för besökets första referent. Använder uppslagstabellen `referrer_type.tsv`. | tinyint unsigned |

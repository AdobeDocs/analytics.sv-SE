---
title: Skicka data till Adobe Analytics med taggtillägget Web SDK
...
# Skicka data till Adobe Analytics med taggtillägget Web SDK

Den här implementeringssökvägen innebär en ny installation av SDK på webben med hjälp av taggar i Adobe Experience Platform Data Collection. Andra implementeringsvägar beskrivs på separata sidor:

* [Webbbibliotek för SDK JavaScript](web-sdk-javascript-library.md): En ny SDK-installation med Web SDK JavaScript-biblioteket (`alloy.js`). Liknar taggtilläggsmetoden för Web SDK (den här sidan), förutom att du själv hanterar implementeringen i stället för att använda tagggränssnittet. Det kräver fältgruppen Adobe Analytics ExperienceEvent, som innehåller typiska analysvariabler som ska inkluderas i XDM-schemat.
* [Analystillägg för Web SDK-tillägg](analytics-extension-to-web-sdk.md): Gå från Adobe Analytics-taggtillägg till Web SDK-taggtillägg på ett smidigt och metodiskt sätt. Detta innebär att man inte behöver använda XDM förrän man är redo att använda Adobe Experience Platform tjänster, som Customer Journey Analytics. Använd objektet `data` i stället för objektet `xdm` för att skicka data till Adobe.
...
![Så här implementerar du Adobe Analytics med hjälp av arbetsflödet för Web SDK-tillägg, enligt beskrivningen i det här avsnittet.](../../assets/websdk-extension-annotated.png)

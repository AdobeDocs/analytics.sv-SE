...
Dessutom kan du begränsa sekventiella segment till en viss tidslängd, granularitet och antal mellan kontrollpunkter med hjälp av operatorerna [Efter och Inom](/help/components/segmentation/segmentation-workflow/seg-sequential-build.md).
...
| Matchar inte | A och sedan C och B (eftersom B inte var inom 1 sida av A)<br>**Obs!** Om dimensionsbegränsningen tas bort, kommer både A och B och A och C sedan B att matcha. |
...
Du kapslar in två sidlinjaler i en [!UICONTROL Visitor]-behållare på den översta nivån och sekventierar sidträffar med operatorn [!UICONTROL THEN].
...
I det här exemplet kapslas två **[!UICONTROL Visit]**-behållare in den översta **[!UICONTROL Visitor]**-behållaren och segmentet sekventieras med hjälp av operatorn [!UICONTROL THEN].
...
Genom att lägga till flera [!UICONTROL Hit]-behållare i en [!UICONTROL Visitor]-behållare kan du använda lämpliga operatorer mellan samma typ av behållare och använda regler och dimensioner som Sida och Besöksnummer för att definiera sidvyn och tillhandahålla en sekvensdimension i [!UICONTROL Hit]-behållaren. Genom att använda logik på träff-nivå kan du begränsa och kombinera matchningar på samma träffnivå i [!UICONTROL Visitor]-behållaren för att skapa en mängd olika segmenttyper.
...
1. Lägg till två [!UICONTROL Hit]-nivåbehållare - en dimension med en lämplig numerisk dimension som förenas på samma [!UICONTROL Hit]-nivå av operatorn [!UICONTROL AND] och [!UICONTROL OR].
1. Lägg till ytterligare en [!UICONTROL Hit]-behållare i [!UICONTROL Visit]-behållaren och kapsla ytterligare två [!UICONTROL Hit]-behållare som är kopplade till en [!UICONTROL OR]- eller [!UICONTROL AND]-operator.

   Sekvens på dessa kapslade [!UICONTROL Hit]-behållare med operatorn [!UICONTROL THEN].

![](assets/aggregate_checkpoints2.png)

## &quot;Kapsling&quot; i sekventiella segment

Genom att placera kontrollpunkter på både [!UICONTROL Visit]- och [!UICONTROL Hit]-nivån kan du begränsa segmentet så att det uppfyller kraven inom ett visst besök samt en viss träff.
...
1. Markera båda reglerna flera gånger, klicka på **[!UICONTROL Options]** > **[!UICONTROL Add container from selection]** och ändra den till en [!UICONTROL Visit]-behållare.
1. Koppla dem till en [!UICONTROL THEN]-operator.
1. Skapa en Träff-behållare som en peer till [!UICONTROL Visit]-behållaren och dra i en siddimension.
...
Segmentregler inkluderar alla data såvida du inte uttryckligen utesluter [!UICONTROL Visitor], [!UICONTROL Visit] eller [!UICONTROL Hit]-data med hjälp av regeln [!UICONTROL Exclude]. Det gör att ni kan avfärda vanliga data och skapa segment med mer fokus. Eller så kan du skapa segment som utesluter hittade grupper för att identifiera den återstående datauppsättningen, till exempel skapa en regel som innefattar framgångsrika besökare som lade order och sedan exkludera dem för att identifiera&quot;icke-köpare&quot;. I de flesta fall är det dock bättre att skapa regler som utesluter breda värden i stället för att använda regeln [!UICONTROL Exclude] för att ange specifika inkluderingsvärden som mål.
...
Skapa ett segment på samma sätt som för ett enkelt, blandat eller kapslat sekventiellt segment och ange sedan operatorn [!UICONTROL EXCLUDE] för behållarelementet. Exemplet nedan är ett aggregeringssegment där de tre [!UICONTROL Hit]-behållarna dras till arbetsytan, där [!UICONTROL THEN]-operatorn är tilldelad att ansluta till behållarlogiken och utelämna mellansidvisningsbehållaren så att endast besökare som gick från sidan A till sidan C inkluderas i sekvensen.
...
[!UICONTROL Logic Group]-behållaren är utformad för att behandla *flera kontrollpunkter som en grupp*, *utan någon ordning* bland de grupperade kontrollpunkterna. Med andra ord, vi bryr oss inte om ordningen på kontrollpunkterna i den gruppen. Du kan till exempel inte kapsla en [!UICONTROL Visitor]-behållare i en [!UICONTROL Visitor]-behållare. I stället kan du kapsla en [!UICONTROL Logic Group]-behållare i en [!UICONTROL Visitor]-behållare med specifika [!UICONTROL Visit]-kontrollpunkter och [!UICONTROL Hit]-kontrollpunkter.

>[!NOTE]
>
>En [!UICONTROL Logic Group] kan bara definieras i ett sekventiellt segment, vilket innebär att operatorn [!UICONTROL THEN] används i uttrycket.

| Behållarhierarki | Illustration | Definition |
|---|---|---|
| Standardbehållarhierarki | ![](assets/nesting_container.png) | I [!UICONTROL Visitor]-behållaren kapslas behållarna [!UICONTROL Visit] och [!UICONTROL Hit] i sekvens för att extrahera segment baserat på träffar, antalet besök och besökaren. |
...
Ja, det kan de. Låt oss anta att du försöker identifiera det här segmentet av besökare: **Besökare som visade sida A och sedan visade sidorna i B, C och D**
...
Precis som andra behållare kan [!UICONTROL Logic Group]-behållare byggas på flera sätt i [!UICONTROL Segment Builder]. Här är ett sätt att kapsla in [!UICONTROL Logic Group]-behållare:

1. Dra mått, händelser eller segment från de vänstra rutorna.
1. Ändra den översta behållaren till en [!UICONTROL Visitor]-behållare.
...
1. Du kan nu ställa in [!UICONTROL Hit] i [!UICONTROL Logic Group]-behållaren utan hänsyn till hierarkin.

### Kontrollpunkter för logikgrupp i vilken ordning som helst

Om du använder [!UICONTROL Logic Group] kan du uppfylla villkoren i gruppen som ligger utanför sekvensen. Detta gör att du kan skapa segment där en [!UICONTROL Visit]- eller [!UICONTROL Hit]-behållare inträffar oavsett den normala hierarkin.

**Exempel**: Besökare som besökt sida A, besökte sedan sida B och sida C i valfri ordning.

**Skapa det här segmentet**

Sida B och C är kapslade i en [!UICONTROL Logic Group]-behållare i den yttre [!UICONTROL Visitor]-behållaren. Behållaren [!UICONTROL Hit] för A följs sedan av behållaren [!UICONTROL Logic Group] med B och C som identifieras med operatorn [!UICONTROL AND]. Eftersom den finns i [!UICONTROL Logic Group] definieras inte sekvensen och om du trycker på både sida B och C i någon ordning blir argumentet true.
...
Om du använder [!UICONTROL Logic Group] kan du uppfylla villkoren i gruppen som ligger utanför sekvensen. I det här osorterade första matchningssegmentet identifieras först [!UICONTROL Logic Group]-reglerna som antingen en sidvy av sida B eller sida C, och därefter som en nödvändig vy av sida A.

**Exempel**: Besökare som besökt antingen sida B eller sida C, besökte sedan sida A.

**Skapa det här segmentet**

Dimensionerna för sida B och C grupperas i en [!UICONTROL Logic Group]-behållare där operatorn [!UICONTROL OR] är markerad och sedan [!UICONTROL Hit]behållaren som identifierar en sidvy av sida A som värdet.

![](assets/logic_group_1st_match.png)

### Logikgrupp exkluderar OCH

Bygg segment med [!UICONTROL Logic Group], där flera sidvyer sammanställs för att definiera vilka sidor som behöver mötas medan andra sidor specifikt missades. ****
...
När du har kapslat värdena i [!UICONTROL Logic Group] klickar du på knappen **[!UICONTROL Exclude]** i [!UICONTROL Logic Group]-behållaren.
...
De första B- och C-sidorna identifieras i en [!UICONTROL Logic Group]-behållare som är exkluderad och följs sedan av en träff till sida A av besökaren.
...
Använd de inbyggda [!UICONTROL Within]- och [!UICONTROL After]-operatorerna i huvudet för varje behållare för att definiera tid, händelser och antal.

![](assets/then_within_operators.png)

Du kan begränsa matchningen till en viss tidslängd genom att använda behållarna [!UICONTROL Within] och [!UICONTROL After] och ange en granularitet och ett antal. Operatorn [!UICONTROL Within] används för att ange en maximal tidsgräns mellan två kontrollpunkter. Operatorn [!UICONTROL After] används för att ange en minimigräns för hur lång tid det tar mellan två kontrollpunkter.
...
* Tid efter kan bara användas för en [!UICONTROL Hit]-behållare eftersom det är den enda nivån för vilken en sådan fin granularitet har definierats.

**Exempel**: Besökare som besökt sida A besökte sedan sida B först efter 2 veckor.***

![](assets/time_between_after_operator.png)

**Skapa segmentet**: Det här segmentet skapas genom att en  [!UICONTROL Visitor] behållare med två  [!UICONTROL Hit] behållare läggs till. Du kan sedan ställa in operatorn [!UICONTROL THEN] och öppna listrutan [!UICONTROL AFTER] och ställa in antalet veckor.
...
|--- |--- |--- |
| **** Ahit: 1 juni 2019 00:01 | **** Bhit: 15 juni 2019 00:01 | **Matchar:** Tidsbegränsningen matchar eftersom den är efter 1 juni 2019 (två veckor). |
| **** Ahit: 1 juni 2019 00:01 | **** Bhit: 8 juni 2019 kl. 00:01 15 juni 2019 00:01 | **Matchar inte:** Den första träffen på sida B matchar inte eftersom den står i konflikt med begränsningen som kräver den efter två veckor. |
...
**Skapa segmentet**: Segmentet skapas genom att en  [!UICONTROL Visitor] behållare läggs till och sedan dras med två  [!UICONTROL Hit] behållare. Du kan sedan ange operatorn [!UICONTROL THEN], öppna listrutan för operatorn [!UICONTROL AFTER] och ange intervallet: träffar, sidvisningar, besök, minuter, timmar, dagar, veckor, månader, kvartal eller år.
...
Alla besökare som besöker sidan A den 1 juni 2019 återvänder efter den 15 juni 2019 00:01, men *före den 1 juli 2019 inkluderas i segmentet.* Jämför med [Tid mellan undantag](/help/components/segmentation/segmentation-workflow/seg-sequential-build.md).

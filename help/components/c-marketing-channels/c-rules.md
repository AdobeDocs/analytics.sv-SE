...
4. Om du vill lägga till en ny regel väljer du den i **[!UICONTROL Add New Rule Set]** -menyn. Om du väljer en kanal får du en regelmall och om du väljer Egen börjar du från en tom skiva. Med båda alternativen kan du ändra regeluppsättningen efter behov.
...
>[!NOTE]
>
>Alla textfält som du definierar, till exempel frågesträngsparametrar eller listor med värden som ska matchas mot, utvärderas som **skiftlägesokänslig** värden. Om du till exempel har en regel där frågesträngsparametern cmp = abc123, kommer alla versioner av både &#39;cmp&#39; och &#39;abc123&#39; att matcha regeln. Du behöver inte lista flera versioner av dessa värden.
...
| Sida | Sidnamnet på en webbsida på din webbplats som är taggad med Adobe webbfyr. Detta värde motsvarar s.pageName . Exempel `Home Page` och `About Us`. |
| Siddomän | Domänen för den sida där besökaren hamnar, t.ex. `products.example.co.uk`. |
...
| Refererande domän | Domänen som besökarna kom från innan de besökte din webbplats, till exempel referenser från `abcsite.com` kontra `xyzsite.com`. |
| Frågesträngsparameter | Om en sidadress på webbplatsen ser ut som `https://example.com/?page=12345&cat=1`och &quot;page&quot; och &quot;cat&quot; är båda frågesträngsparametrar. (Se `https://en.wikipedia.org/wiki/Query_string`.)  Du kan bara ange en frågesträngsparameter per regeluppsättning. Om du vill lägga till ytterligare frågesträngsparametrar använder du `ANY` som operatorn och lägg sedan till nya frågesträngsparametrar i regeln. Frågesträngsparametrar utvärderas som skiftlägeskänsliga; &quot;cat&quot; och &quot;CAT&quot; kommer till exempel att utvärderas på samma sätt. |
...
| Refererande parameter | En frågesträngsparameter på referensens URL. Om besökarna kommer från `example.com/?page=12345&cat=1`, så är page och cat de refererande parametrarna. |
...
För marknadsföringskanalregeln gäller följande: [!UICONTROL Paid Search] är följande:

![](assets/example_paid_search.png)

Se [Betalsökningsidentifiering](https://experienceleague.adobe.com/docs/analytics/admin/admin-tools/paid-search-detection/paid-search-detection.html) i Admin om du vill ha mer information.
...
Det finns ingen naturlig sökidentifiering i Analytics. När du har konfigurerat betald sökidentifiering vet systemet att om en sökreferent inte var en betalande sökreferent måste det vara en naturlig sökreferent. Se [Betalsökningsidentifiering](https://experienceleague.adobe.com/docs/analytics/admin/admin-tools/paid-search-detection/paid-search-detection.html) i Admin för mer information.

...
Använd den här ordlistan för att förstå sammanhanget för många termer som används i Adobe Analytics.
...
* **Admin Console:** Kan referera till:
   ...
   * Adobe Admin Console, där produktåtkomst tillhandahålls och användarbehörigheter hanteras. See [Admin Console](/help/admin/admin-console/home.md) in the Admin user guide.
...
* **ASI-kortplats:** Finns inte längre. I tidigare versioner av Adobe Analytics har ASI-kortplatserna en temporär rapportsvitsbehållare för att visa segmenterade data. I den aktuella versionen av Adobe Analytics kan segment tillämpas direkt på alla rapporter.
...
* **Campaign:** Kan referera till:
   ...
   * En standardklassificering av spårningskod. skapas automatiskt för alla rapportsviter.
...
* **Kanal:** Kan referera till:
   * Variabeln Kanal, som fyller i dimensionen Platsavsnitt. Se [Sidvariabler](/help/implement/vars/page-vars/page-variables.md) i Implementera användarhandbok.
...
* **Klassificering:** En funktion i Adobe Analytics som gör det möjligt att gruppera dimensionsobjekt. Se [Klassificeringar](/help/components/c-classifications2/c-classifications.md) i användarhandboken för komponenter.
...
* **Kundsupport:** En utsedd användare som har rätt att interagera direkt med Adobes kundtjänst. Se [Kundsupportrepresentanter](https://helpx.adobe.com/experience-cloud/supported-users.html) i Experience Cloud Knowledgebase.
...
* **Dynamisk tagghantering:** Adobes tidigare tagghanteringslösning. Se Översikt över [DTM-implementering](/help/implement/other/dtm/dtm-implementation-overview.md) i användarhandboken för implementering. Adobe rekommenderar att du använder Adobe Experience Platform Launch i stället.
...
* **ExcelClient:** Används inte längre som term. Namnet på den som föregår Report Builder.
...
* **H-kod:** En föregångare till AppMeasurement. I tidigare versioner av Adobe Analytics mättes kodversionerna med&quot;H version&quot;, till exempel H.27.5, H.26.
* **Träff:** En enda bildbegäran som skickas till Adobes datainsamlingsservrar. Både sidvyer och anpassade länkar kan kallas träffar.
* **Bildbegäran:** En genomskinlig 1x1-pixelbild som används för att kommunicera med Adobes datainsamlingsservrar. En webbplats begär denna osynliga bild med en lång frågesträng som innehåller data. Adobe returnerar den osynliga bilden och tolkar frågesträngen som tagits emot.
* **Insight:** Kan referera till:
   * Datans Workbench tidigare namn.
   * Custom Insight, ett historiskt namn för en anpassad trafikvariabel.
* **KPI:** Förkortning för nyckelresultatindikator. Mätvärden som hjälper ett företag att förstå hur webbplatsen fungerar. Varje organisation har olika nyckeltal som mäter olika aspekter av sin verksamhet. Se [Skapa ett dokument](/help/implement/prepare/solution-design.md) för lösningsdesign i användarhandboken för Implementera.
...
* **Starta:** Short for Adobe Experience Platform Launch, Adobe&#39;s current implementation solution. Se [Översikt](https://docs.adobe.com/content/help/en/launch/using/overview.html) i användarhandboken för Adobe Experience Platform Launch.
...
* **Förekomster:** En typ av mätvärde som visar hur många träffar en dimensionsuppgift har angetts eller beständig. Se [förekomstmåttet](/help/components/metrics/occurrences.md) i användarhandboken för komponenter.
* **Omniture:** Används inte längre som term. Organisationen som ägde Adobe Analytics innan det förvärvades av Adobe 2009.
* **Banor:** Se Flöde.
...
* **Bearbetar regler:** Kan referera till:
   * Bearbetningsregler är ett sätt att ändra datainsamling med vissa regler i Admin Console. Se [Bearbeta regler](/help/admin/admin/c-processing-rules/processing-rules.md) i användarhandboken för Admin.
   * Bearbetningsregler för marknadsföringskanaler, en uppsättning regler som avgör vilken marknadsföringskanal en träff tillhör. Se Bearbetningsregler [för](/help/admin/admin/marketing-channels-admin.md) marknadsföringskanaler i användarhandboken för Admin.
...
* **s.t():** Namnet på funktionen i ett AppMeasurement-bibliotek som skickar en begäran om sidvisningsbild. Vissa AppMeasurement-bibliotek använder `s.track()` i stället. Se [det](../implement/vars/functions/t-method.md) i Implementera användarhandbok.
* **s<span>.</span>tl():** Namnet på funktionen i ett AppMeasurement-bibliotek som skickar en begäran om länkspårningsbild. Vissa AppMeasurement-bibliotek använder `s.trackLink()` i stället. Se [tl](../implement/vars/functions/tl-method.md) i Implementera användarhandbok.
* **s_code.js:** Namnet på den JavaScript-fil som används i tidigare versioner av Adobe Analytics. Det aktuella namnet på JavaScript-filen som används är AppMeasurement.js.
